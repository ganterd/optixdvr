...
NK_API int                  nk_glfw3_create_texture_rgba32f(const void* image, int width, int height);
...
NK_API int
nk_glfw3_create_texture_rgba32f(const void* image, int width, int height)
{
    GLuint id;
    GLsizei w = (GLsizei)width;
    GLsizei h = (GLsizei)height;
    ...
    glTextureStorage2D(id, 1, GL_RGBA8, w, h);
    if (image)
        glTextureSubImage2D(id, 0, 0, 0, w, h, GL_RGBA,  GL_FLOAT, image);
    else 
